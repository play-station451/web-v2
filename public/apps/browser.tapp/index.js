...
function customDecode(encodedString) {
	...
	let decodedString = decodeURIComponent(firstPart)
		.split("")
		.map((char, index) =>
			index % 2 ? String.fromCharCode(2 ^ char.charCodeAt(0)) : char,
		)
		.join("");
	let finalResult =
		decodedString + (restParts.length ? "?" + restParts.join("?") : "");
	return finalResult;
}

const topbar_height = document
...
function newTab() {
		...
		if (e.key === "Enter") {
			...
			switch (url) {
				...
				default:
					const input = url;
					Filer.promises
						.readFile(`/home/${user}/settings.json`, "utf8")
						.then(async (data) => {
							...
							const isUrl =
								/^(https?:\/\/)|(localhost(:\d+)?([\/?]|$))|([a-z0-9\-]+\.[a-z]{2,})/i.test(
									input,
								) && !/\s/.test(input);
							let targetUrl;
							if (isUrl) {
								targetUrl = input.startsWith("http")
									? input
									: `https://${input}`;
							} else {
			...
			activeTabContent.onload = () => {
				const pageTitle =
					activeTabContent.contentDocument.title || "Untitled";
				const maxTitleLength = 8;
				const tabTitle =
					pageTitle.length > maxTitleLength
			...
			if (localStorage.getItem("defUrl") === "about:newtab") {
				...
				tab_content.addEventListener("load", () => {
					tab_content.contentWindow.addEventListener("updTab", () => {
						updateTab = false;
						tab_content.contentWindow.removeEventListener(
							"updTab",
							arguments.callee,
						);
					});
				...
				if (proxy === "Ultraviolet") {
					tab_content.src =
						...
						"/uv/service/" +
						customEncode(
							localStorage.getItem("defUrl") || "about:newtab",
						);
	...
	const inject_engines = () => {
		if (
			document.querySelector(".left-arrow").classList.contains("disabled")
		) {
		...
		if ("__uv$location" in tab_content.contentDocument) {
			...
			if (updateTab === false) {
				urlbar.value = window.parent.$scramjet.codec.decode(
					tab_content.contentWindow.window.location.href.replace(
						/^.*\/service\//,
						"",
					),
				);
		...
		if (
			!tab_content.contentDocument.getElementById("tb-cursor-controller")
		) {
		...
		if (
			!tab_content.contentDocument.getElementById("tb-media-controller")
		) {
		...
		if (
			!tab_content.contentDocument.getElementById("userscript-container")
		) {
			const userscript_container = document.createElement("div");
			userscript_container.id = "userscript-container";
			Filer.promises
				.readFile(
					`/apps/user/${sessionStorage.getItem("currAcc")}/browser/userscripts.json`,
				)
				.then(async (data) => {
					...
					for (const script of dat) {
						...
						if (getPat(urlbar.value, script.match)) {
							...
							user_script.text = await Filer.promises.readFile(
								script.file,
								"utf8",
							);
							user_script.type = "text/javascript";
							user_script.id = `userscript-${script.name}`;
							user_script.setAttribute(
								"data-script-path",
								script.file,
							);
...
document.querySelector(".ext-btn").addEventListener("click", () => {
	const activeTabContent = document.querySelector(".tab-content.active");
	activeTabContent.contentWindow.location.href =
		"/apps/browser.tapp/extensions.html";
...
document.querySelector(".fav-button").addEventListener("click", async () => {
	...
	const dat = JSON.parse(
		await Filer.promises.readFile(
			`/apps/user/${await window.parent.tb.user.username()}/browser/favorites.json`,
			"utf8",
		),
	);
	const favicon =
		activeTabContent.contentDocument.querySelector("link[rel~='icon']")
			?.href ||
		activeTabContent.contentDocument.querySelector(
	...
	dat.push({
		...
		icon: favicon,
		url: await tb.proxy.decode(
			activeTabContent.src
...
const pwaIns = async () => {
	...
	const favicon =
		...
		"/apps/browser.tapp/icon.svg";
	let data = JSON.parse(
		await Filer.promises.readFile("/apps/web_apps.json", "utf8"),
	);
	await Filer.exists(
		`/apps/user/${await window.parent.tb.user.username()}/${tabTitle}/index.json`,
		async (exists) => {
			let apps = data.apps;
			if (apps.includes(tabTitle.toLowerCase()) || exists) {
				...
				await new Filer.Shell().promises.rm(
					`/apps/user/${await window.parent.tb.user.username()}/${tabTitle}`,
					{ recursive: true },
				);
			} else {
...
const showTabs = () => {
	if (document.querySelector(".tab-container").style.display === "none") {
		...
		document.querySelector("main").style.height =
			`calc(100% - calc(var(--topbar-height) + 10px))`;
