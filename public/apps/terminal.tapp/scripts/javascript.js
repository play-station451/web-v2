async function JS(args) {
	if (args.length === 0 || args[0] === "javascript") {
		...
		try {
			...
			const scriptContent = await fetch(scriptPath).then(response => response.text());
			const modifiedScript = scriptContent.replace(/console\.log\(([^)]+)\);?/g, "displayOutput($1);").replace(/console\.error\(([^)]+)\);?/g, "displayError($1);");
			eval(modifiedScript);
		} catch (error) {
			displayError(error);
		}
