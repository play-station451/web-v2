...
const cm = async (e) => {
    ...
    if(e.target.getAttribute("type") === ("file")) {
                        ...
                        try {
                            ...
                            await window.parent.tb.dialog.Permissions({
                                ...
                                onOk: async () => {
                                    ...
                                    const appName = path.replace(`/home/${window.parent.sessionStorage.getItem('currAcc')}/`, '').replace(/\//g, '.').replace(/\.zip$/, '') + '';
        ...
        options = [
            ...
            //     click: async () => {}
            // } : null,
            e.target.getAttribute("path") === "/system/trash" || e.target.getAttribute("system-folder") === "true" ? null : {
                text: isTrash ? "Delete" : "Move To Trash",
                click: async () => {
                    const path = e.target.getAttribute("path");
                    if(document.querySelector(".exp").getAttribute("path") === "/system/trash") {
