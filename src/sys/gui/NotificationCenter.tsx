...
export default function NotificationCenter() {
    ...
    return (
        <img alt="notifimg" ref={iconRef} src={`/assets/img/notif_${notificationCount > 9 ? 'plus' : Math.max(0, Math.min(notificationCount, 9))}.svg`} className="tooltip_item w-6 h-6 cursor-pointer duration-150 select-none" onMouseUp={() => {
...
const NotificationMenu = ({ isOpen }: INotificationProps) => {
    ...
    return (
        <div ref={notificationCenterRef} className={`
                    ...
                    {notifications.map((notification, index) => {
                        ...
                        return (
                            ...
                            <div className="flex justify-between items-center bg-[#ffffff20] p-2.5">
                                ...
                                {
                                    notification.time ? (
                                        <div className="font-semibold text-[#ffffffa0] text-sm">{time}</div>
                                    ) : null
                                }
                            </div>
                            <div className="flex flex-col gap-2 p-2.5">
                                ...
                                <div className="flex gap-2 justify-between">
                                    <button className="leading-none p-2.5 cursor-pointer px-4 bg-[#ffffff20] shadow-tb-border rounded-md hover:bg-[#ffffff30] duration-150"
                                    onClick={() => dismiss(index)}>Dismiss</button>
                                    <button className="leading-none p-2.5 cursor-pointer px-4 bg-[#53f67463] shadow-tb-border rounded-md hover:bg-[#53f67473] duration-150"
                                    onClick={async () => {
