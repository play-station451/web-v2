...
const WindowElement: React.FC<WindowProps> = ({ className, config, onSnapDone, onSnapPreview }) => {
    ...
    useEffect(() => {
        ...
        const debugCTX = (e: MouseEvent) => {
            ...
            window.tb.contextmenu.create({
                ...
                y: rect.top + 40,
                options: [
                    { text: "Minimize", click: () => { setMinimized(true) } },
                    { text: "Maximize", click: () => { setMaximized(true) } },
                    { text: "Reload", click: () => { if (srcRef.current?.contentWindow) {
                        srcRef.current.contentWindow.location.reload();
                        Object.assign(srcRef.current?.contentWindow as any, {
                            tb: window.parent.tb,
                            anura: window.parent.anura,
                    ...
                    { text: "Close", click: () => { windowStore.removeWindow(wid); clearInfo(); } }
                ]
            })
        }
        const minall: any = () => {
    ...
    return (
        // @ts-ignore
        <div ref={windowRef} message={message} id={wid} pid={pid} className={`
            ${className ? className : ""}
            absolute
            bg-[#ffffff18]
        ...
        `} style={{
            left: maximized ? "" : x, top: maximized ? "" : y,
            ...
            <div className="absolute left-0 top-0 size-full rounded-lg backdrop-blur-[20px] pointer-events-none shadow-tb-border -z-1 bg-[#00000048]" style={{
                backgroundImage: "url(/assets/img/grain.png)",
            }}></div>
            <div ref={focuserRef} className={`absolute rounded-lg ${config.focused ? "inset-x-2 top-[calc(40px+0.5rem)] bottom-2 pointer-events-none opacity-0" : "inset-x-[1px] top-[40px] bottom-[1px] backdrop-blur-[4px] opacity-100"} duration-150`} onMouseDown={() => {
                windowStore.arrange(wid);
                // @ts-ignore
                setZIndex(windowStore.getWindow(wid)?.zIndex);
            }}></div>
            ...
            <div ref={regionRef} className="region flex justify-between items-center bg-[#ffffff10] p-2 min-w-[224px] select-none" onMouseDown={(e: React.MouseEvent) => {
                windowStore.arrange(wid);
                // @ts-ignore
                setZIndex(windowStore.getWindow(wid)?.zIndex);
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <span ref={titleRef} className="font-[680] pointer-events-none">{title}</span>
                ...
                </div>
                {
                    controls ? (
                        <div className="controls flex gap-1">
                            {
                                controls?.map((control, index) => {
                                    if(control === "minimize") {
                                        return (
                                            <svg ref={miniRef} key={index} className={`group size-4 ${config.minimizable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                                ...
                                                <rect className={`
                                                    ${
                                                        config.minimizable === false ? "fill-[#ffffff60]" : "fill-[#ffffffbb] group-hover:fill-white"
                                                    } duration-150 pointer-events-none
                                                `} x="4" y="10" width="16" height="3" rx="2" />
                                            </svg>
                                        )
                                    } if(control === "maximize") {
                                        return (
                                                <svg ref={minMaxRef} key={index} className={`group size-4 ${config.maximizable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                                ...
                                                }}>
                                                    {
                                                        maximized ? (
                                                            <>
                                                                <path className={`
                                                                    ${
                                                                        config.maximizable === false ? "fill-[#ffffff60]" : "fill-[#ffffffbb] group-hover:fill-white"
                                                                    } duration-150 pointer-events-none
                                                            ...
                                                            `} fillRule="evenodd" clipRule="evenodd" d="M8 4C5.79086 4 4 5.79086 4 8V16C4 18.2091 5.79086 20 8 20H16C18.2091 20 20 18.2091 20 16V8C20 5.79086 18.2091 4 16 4H8ZM8 6C6.89543 6 6 6.89543 6 8V16C6 17.1046 6.89543 18 8 18H16C17.1046 18 18 17.1046 18 16V8C18 6.89543 17.1046 6 16 6H8Z" />
                                                        )
                                                    }
                                                </svg>
                                        )
                                        ...
                                        return (
                                            <svg ref={closeRef} key={index} className={`group size-4 ${config.closable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                                ...
                                                <path className={`
                                                    ${
                                                        config.closable === false ? "stroke-[#ffffff60]" : "stroke-[#ffffffbb] group-hover:stroke-white"
                                                    } duration-150 pointer-events-none
                                                `} d="M6 18L18 6M6 6L18 18" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                                            </svg>
                                        )
                                    }
                                })
                        ...
                        <div className="controls flex gap-1">
                            <svg ref={miniRef} className={`group size-4 ${config.minimizable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                if(config.minimizable === false) return;
                                if (windowRef.current) {
                                    windowRef.current.style.transitionProperty = "transform, opacity";
                                    windowRef.current.style.transitionDuration = "150ms";
                            ...
                            <svg ref={minMaxRef} className={`group size-4 ${config.maximizable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                if(config.maximizable === false) return;
                                if (windowRef.current) {
                                    windowRef.current.style.transitionProperty = "width, height, left, top";
                                    windowRef.current.style.transitionDuration = "150ms";
                            ...
                            <svg ref={closeRef} className={`group size-4 ${config.closable === false ? "cursor-default" : "cursor-pointer"} no-drag`} viewBox="0 0 24 24" fill="none" onMouseDown={() => {
                                if(config.closable === false) return;
                                if(windowRef.current) {
                                    windowRef.current.style.transitionProperty = "transform, opacity";
                                    windowRef.current.style.transitionDuration = "150ms";
                ...
                <div ref={contentRef}>
                    <iframe key={config.src} ref={srcRef} src={src} sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-presentation allow-downloads" onLoad={() => {
                        if (config.message) {srcRef.current?.contentWindow!.postMessage(config.message, "*")}
                    ...
                    }} referrerPolicy="no-referrer" style={{border: "none", all: "initial", width: "100%", height: "calc(100% - 40px)", pointerEvents: isMouseDown ? "none" : "auto", userSelect: "none"}}></iframe>
                </div>
            </div>
    )
}
        ...
        if (dragging && draggedItemIndex !== null) {
            ...
            setItems((prevApps) =>
                prevApps.map((app, index) =>
                    index === draggedItemIndex.current
                        ? { ...app, position: { ...app.position, left: newX, top: newY, custom: true } }
                        : app
                )
            )
        }
    ...
    const savePos = async (item: string, left: number, top: number) => {
        try {
            ...
            if (itemIndex !== -1) {
                ...
                if (
                    (Math.abs(Math.round(currentLeft) - Math.round(left)) > 67 || Math.abs(Math.round(currentTop) - Math.round(top)) > 67) &&
                    (Math.round(currentLeft) !== Math.round(left) || Math.round(currentTop) !== Math.round(top))
                ) {
        ...
        <div className="flex gap-1 flex-wrap h-full">
            {
                items.map((item: DesktopItem, i: any) => {
                    return (
                        item.type === "file" ? (
                            <div title={item.name} key={`${item.name}`} id="desktop-item" className="group relative size-max min-w-16 min-h-16 flex flex-col items-center justify-center p-2 text-sm font-medium text-wrap select-none"
                            onDoubleClick={async () => {
                                ...
                                handlers = Object.entries(handlers).filter(([type, app]) => {
                                    return !(type === "text" && app === "text-editor") &&
                                        !(type === "image" && app === "media-viewer") &&
                                        !(type === "video" && app === "media-viewer") &&
                                        !(type === "audio" && app === "media-viewer");
                                });
                                let hands = []
                            ...
                            onContextMenuCapture={(e: React.MouseEvent<HTMLDivElement>) => {
                                ...
                                window.tb.contextmenu.create({
                                    ...
                                    options: [
                                        ...
                                        { text: "Delete Shortcut", click: async () => {
                        ...
                        <div data-type="shortcut" title={item.name} key={`${item.name}`} id="desktop-item" className="group relative size-max min-w-16 min-h-16 flex flex-col items-center justify-center p-2 text-sm font-medium text-wrap select-none" onDoubleClick={() => {
...
const WindowArea: React.FC<WindowAreaProps> = ({ className }) => {
    ...
    return (
        // @ts-ignore
        <window-area class={`${className ?? className} relative`} onContextMenuCapture={(e: MouseEvent) => {
                        ...
                        click: async () => {
                            const make = async (item: any) => {
                                ...
                                if (aname.includes(".tapp")) {
                                    ...
                                    await Filer.fs.promises.writeFile(`${item}/desktopcfg.json`, JSON.stringify({
