...
const Desktop: FC<IDesktopProps> = ({ desktop, onContextMenu }) => {
    ...
    return (
        <div className={`desktop flex flex-col h-[inherit] overflow-hidden `} style={{
            backgroundImage: `url(${wallpaper})`,
            backgroundSize: wallpaperMode === "stretch" ? "100% 100%" : wallpaperMode,
            backgroundPosition: "center",
            backgroundRepeat: "no-repeat"
        }} data-desktop={desktop} ref={desktopRef} onContextMenuCapture={(e: React.MouseEvent<HTMLDivElement>) => {onContextMenu?.(e.nativeEvent)}}>
            <Shell />
                ...
                {winPrev?.windows && winPrev.windows.length > 0 && (
                    <div className={`absolute bottom-16 flex flex-col justify-center items-center rounded-lg bg-[#2020208c] shadow-tb-border-shadow backdrop-blur-[100px] border-none overflow-hidden z-9999999`} style={{ left: `calc(${winPrev?.location}px - ${45 * (winPrev?.windows.length)}px)`}}>
                        {winPrev.windows[0].map((win: WindowConfig) => (
                            <div key={win.wid} className={`flex justify-center items-center p-2 gap-14 hover:bg-[#ffffff10] ${win.pid === String(window.tb.window.getId()) ? "bg-[#ffffff18]" : ""} duration-150`} onClick={() => {
                                ...
                                <svg className="size-6 p-0.5 rounded-sm cursor-pointer text-[#ffffffbb] no-drag hover:text-white hover:bg-[#ffffff10]" viewBox="0 0 24 24" fill="none" onClick={() => {
