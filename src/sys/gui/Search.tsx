...
const SearchMenu: FC<SearchProps> = ({ className, searchRef }) => {
    ...
    return (
        <div ref={searchMenuRef} className={className + `
            bg-[#2020208c] shadow-tb-border-shadow backdrop-blur-sm rounded-xl
            flex flex-col items-center justify-between
            min-w-[440px] h-[266px]
        `}>
            <div className={"flex gap-2 items-center text-[#ffffffa4] p-2.5 pb-0 w-full duration-700" + " " + `
                ${
            ...
            `}>
                ...
                <div className="relative flex items-center w-full">
                    <span ref={placeholderRef} className={`absolute font-[680] text-lg pointer-events-none duration-150 ${searchHasText ? "opacity-0 -translate-x-1.5" : searchActive ? "opacity-100" : "opacity-75"}`}>Search for apps and files</span>
                    <input ref={searchRefRef} type="text" className="bg-transparent focus-visible:outline-none text-lg font-[680] w-full cursor-text"
                            ...
                            if(value.length > 0) {
                                ...
                                if (appres && Array.isArray(appres) && appres.length > 0) {
                                    ...
                                    const appName = typeof app.name === 'string'
                                        ? app.name
                                        : (app.name && typeof app.name.text === 'string'
                                            ? app.name.text
                                            : '');
                                    setResults([[{
                                        icon: iconHtml,
                                                ...
                                                return {
                                                    ...
                                                    onClick: async () => {
                                                        ...
                                                        await window.tb.dialog.Select({
                                                            ...
                                                            options: [
                                                                { text: "Text Editor", value: "text" },
                                                                { text: "Media Viewer", value: "media" },
                                                                { text: "Webview", value: "webview" },
                                                                ...hands,
                                                                        ...
                                                                        parent.window.tb.dialog.DirectoryBrowser({
                                                                            ...
                                                                            onOk: async (val: string) => {
                                                                                const app = JSON.parse(
                                                                                    await Filer.fs.promises.readFile(`${val}/.tbconfig`, "utf8")
                                                                                );
            ...
            <div className={"relative flex items-center min-h-[104px] h-full w-full gap-2 p-2.5 pt-0 duration-1000" + " " + `
                ${searchMenuStore.open ? "" : "translate-y-4 opacity-0"}`
            }>
                <div ref={containerRef} className={"grid gap-2 pt-2 w-full h-full overflow-hidden"}>
                    <div ref={recentAppsRef} className={"relative grid overflow-hidden" + " " + `
                        ${recentApps.length > 0 ? "" : "items-center justify-center"}
                    `}>
                        {
                            recentApps.length <= 0 &&
                            <h1 className={"font-bold text-lg leading-none pt-2 text-[#ffffff68]" + " " + `
                        ...
                        `}>
                            ...
                            <div className={"grid items-center gap-1 overflow-y-auto rounded-md" + " " + `
                                ${results.length > 0 ? "grid-cols-1" : "grid-cols-2"}
                            `}>
                                {
                                    recentApps.length > 0 ? recentApps.sort((a: any, b: any) => {
                                        const valueDiff = (b.value ?? 0) - (a.value ?? 0);
                                        if (valueDiff !== 0) return valueDiff;
                                        return (b.weight ?? 0) - (a.weight ?? 0);
                                    }).slice(0, 8).map((app: any, i: number) => (
                                            <StartItem key={i} className="w-full" title={app.title} icon={app.icon} pid={undefined} src={app.src} onClick={() => {
                                                createWindow({
                                                    src: app.src,
                                                    size: app.size,
                    ...
                    <div ref={resultRef} className={"flex flex-col p-2 bg-[#15151594] rounded-lg shadow-tb-border-shadow overflow-y-auto" + " " + `
                                    ...
                                    <div className={"flex flex-col gap-0.5 overflow-y-auto"}>
                                        {
                                            Array.isArray(results[0]) && results[0].length > 0 ?
                                                results[0].map((app: any, i: number) => (
                                                    <div key={i} className={"flex flex-col gap-2 search-result-app cursor-pointer hover:bg-[#22222288] rounded-md p-2"} onClick={() => { app.click() }}>
                                                        <div className={"flex flex-row gap-1 items-center"}>
                                                            {
                                                                typeof app.icon === "string"
                                                                    ? app.icon.trim().startsWith("<svg")
                                                                        ? <span dangerouslySetInnerHTML={{ __html: app.icon }} />
                                                                        : <img className="w-[49px] h-[49px]" src={app.icon.replace(/^<img.*src="([^"]*)".*$/, "$1")} alt={app.name} />
                                                                    : app.icon
                                                            }
                                                            <div>
                                                ...
                                                results[1].map((f: any, i: number) => (
                                                    <div key={i} className={"flex flex-col gap-2 search-result-file cursor-pointer hover:bg-[#22222288] rounded-md p-2"} onClick={() => { f.onClick() }}>
                                                        <div className={"flex flex-row gap-1 items-center"}>
                                                            {
                                                                typeof f.icon === "string"
                                                                    ? f.icon.trim().startsWith("<svg")
                                                                        ? <span dangerouslySetInnerHTML={{ __html: f.icon }} />
                                                                        : <img className="w-[49px] h-[49px]" src={f.icon.replace(/^<img.*src="([^"]*)".*$/, "$1")} alt={f.name} />
                                                                    : f.icon
                                                            }
                                                            <div>
                ...
                </div>
                {
                    ...
                    <div className={"absolute top-1/2 left-1/2 -translate-1/2 flex gap-1.5 duration-150 items-center text-[#ffffff51]" + " " + `
